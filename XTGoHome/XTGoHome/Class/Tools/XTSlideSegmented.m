...
#import "XTSlideSegmented.h"
#import "XLSlideSegmentedItem.h"


//最大放大倍数
static const CGFloat ItemMaxScale = 1.1;
@interface XTSlideSegmented ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation XTSlideSegmented
- (instancetype)init {
    if (self = [super init]) {
        [self buildUI];
    }
...
- (void)buildUI {
 ...
 layout.minimumLineSpacing = 50;
    ...
    _bottomLine.backgroundColor = [XTColor colorWithHexString:BACK_COLOR];
    _bottomLine.frame = CGRectMake(0, _collectionView.bounds.size.height - 0.5, self.bounds.size.width, 0.5);
   // [self addSubview:_bottomLine];
//    self.layer.borderWidth = 1;
//    self.layer.borderColor = [XTColor colorWithHexString:BACK_COLOR].CGColor;
...
- (void)layoutSubviews {
    ...
    float height=40-1.0f;
    
    float width=self.bounds.size.width;
    
    bottomBorder.frame = CGRectMake(0.0f, height, width, 1.0f);
    bottomBorder.backgroundColor = [XTColor colorWithHexString:BACK_COLOR].CGColor;
    
    [self.layer addSublayer:bottomBorder];
        self.layer.shadowColor = [XTColor colorWithHexString:PINK_COLOR].CGColor;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    item.textLabel.font = [UIFont fontWithName:FONT_REGULAR size:15];
...
- (CGFloat)itemWidthOfIndexPath:(NSIndexPath*)indexPath {
    ...
    NSDictionary *attributes = @{ NSFontAttributeName : [UIFont fontWithName:FONT_REGULAR size:15], NSParagraphStyleAttributeName : style };
...
- (CGRect)shadowRectOfIndex:(NSInteger)index {
    return CGRectMake([_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]].frame.origin.x, self.bounds.size.height - 6, [self itemWidthOfIndexPath:[NSIndexPath indexPathForRow:index inSection:0]], 2);
}
